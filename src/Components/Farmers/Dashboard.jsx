...
export default function DashBoard() {
    ...
    useEffect(() => {
        const fetchWeatherData = async () => {
          try {
            const apiKey = 'd322af5a4337bc54811cc102c6d3bf64';
    ...
    return (
        <>
            ...
            <div className="w-[100%] z-10 h-max py-2 max-xl:px-0 px-5 mr-0 bg-white drop-shadow-2xl">
                ...
                <button className="block max-md:hidden w-max px-4 text-lg h-8 rounded bg-green-500 hover:bg-[#ebeb5a] absolute right-0 top-0 mt-2 mx-8">
                    Logout
                </button>
            ...
            <div className='flex flex-row z-5 bg-gray-100 max-md:pb-16'>
                <div className="hidden md:block h-[700px] w-[300px] max-sm:w-[90%] bg-white text-xl border rounded pl-2 pt-4 ">
                    ...
                    <div className='flex flex-col text-lg m-8 list-none' >
                        ...
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer' onClick={() => navigate("/add")} >Add  for bidding</li>
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer' onClick={() => navigate("/myp")}>Products</li>
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer' onClick={() => navigate("/orders")}>orders</li>
                ...
                <div className={nav ? 'fixed left-0 top-0 w-[90%] h-full border-r  bg-gray-200 ease-in-out duration-500 p-5 text-2xl  z-40 md:hidden' : 'ease-in-out duration-500  fixed left-[-100%] md:hidden'} onClick={handleNav}>
                    ...
                    <div className='flex flex-col text-xl m-8 list-none' >
                        <Link onClick={handleHome} to='/Admin' className='hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer'>Home</Link>
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer ' onClick={() => navigate("/add")} >ADD PRODUCT FOR BIDDING</li>
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer' onClick={() => navigate("/myp")}>Products</li>
                        <li className='mt-4 hover:underline hover:underline-offset-4 decoration-yellow-400 cursor-pointer' onClick={() => navigate("/orders")}>orders</li>
                        <button onClick={handleLogOut} className="w-max mt-16 px-4 text-lg h-8 rounded text-white bg-green-700 hover:bg-[#ebeb5a]">
                            Logout
                        </button>
                    </div>
                </div>
                ...
                <div className='w-[100%] h-[650px] max-lg:h-max  rounded text-center max-md:pb-16' >
                    ...
                    <div className='mt-10 w-[100%] grid gap-8 gap-y-16 max-lg:pb-16 lg:grid-cols-2 max-lg:grid-rows-4 justify-items-center' >
                        <div className='  w-[70%] h-[200px] text-center border-2 text-2xl rounded-md  ' >
                            <div className='flex'>
                                <img src={img1} alt='dd' className='h-[20vh] ' />
                                <h1 className=' flex text-center justify-center items-center'> 10 c</h1>
                            </div>
